...
namespace RHI {
    using enum Format;
    std::unordered_map<RHI::Format, int> vk_map = {
    ...
    Format FormatFromVk(int vk_format)
    {
        if(auto it = std::find_if(vk_map.begin(), vk_map.end(), [vk_format](auto&& pair){
            return pair.second == vk_format;
        }); it != vk_map.end())
        {
            return it->first;
        }
    ...
    int FormatToVk(Format f)
    {
        if(vk_map.contains(f)) return vk_map.at(f);
